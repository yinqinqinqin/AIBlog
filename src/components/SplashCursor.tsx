...
function SplashCursor({
  ...
  useEffect(() => {
    ...
    function getWebGLContext(target) {
      ...
      if (!isWebGL2) {
        context = target.getContext("webgl", params) || target.getContext("experimental-webgl", params);
      }
      if (!context) {
        return { gl: null, ext: {} };
      }
      ...
      const halfFloatTexType = isWebGL2 ? context.HALF_FLOAT : halfFloat && halfFloat.HALF_FLOAT_OES;
      let formatRGBA;
      let formatRG;
      let formatR;
    ...
    function hexToRGB(hex) {
      ...
      const r = Number.parseInt(val.slice(0, 2), 16) / 255;
      const g = Number.parseInt(val.slice(2, 4), 16) / 255;
      const b = Number.parseInt(val.slice(4, 6), 16) / 255;
